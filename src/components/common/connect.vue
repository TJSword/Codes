...
export default {
  data(){
    return{
      menus:[ //菜单
        { label:'首页',icon:'menu',routerUrl:'/menuindex',index:' ' }
      ],
  ...
  methods:{
    ...
    toConnect(item){
      ...
      if(item.index!=' '){
        ...
        this.$store.dispatch("setQueryMenuIndex",JSON.parse(item.index))
        sessionStorage.setItem('queryMenuIndex', JSON.parse(item.index));
      }
      if(item.label==='数据可视化'){
