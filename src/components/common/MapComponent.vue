<template>
  <div class="map_box">
    <!-- 地图工具1 -->
    <div class="map_tool">
      <!-- <div class="tool_item" @click="interceptState = !interceptState">
        <el-tooltip class="item" effect="light" content="地图截取" placement="bottom-end">
          <i class="iconfont icon-jiequ" :style="{'color':interceptState?'#f56c6c':'#333'}"></i>
        </el-tooltip>
      </div> -->
      <div class="tool_item" @click="showMapSelect = !showMapSelect,layerSetting = false">
      ...
      <div class="tool_item" @click="layerSetting = !layerSetting,showMapSelect = false">
        <el-tooltip class="item" effect="light" content="图层设置" placement="bottom-end">
          <i class="iconfont icon-tucengshezhi"  :style="{'color':layerSetting?'#f56c6c':'#333'}"></i>
        </el-tooltip>
      </div>
    </div>
    ...
    <div class="tool_box">
      ...
      <!-- 地图截取 -->
      <div class="tool_intercept" v-if="interceptState">
        <el-radio-group v-model="interceptType">
          <el-radio label="range">范围截取</el-radio>
          <el-radio label="center">中心截取</el-radio>
        </el-radio-group>
        <div class="box" v-show="interceptType=='range'">
          <div class="item">
            <label class="config_label">顶点1</label>
            <el-input size="mini" style="width:180px;" v-model="lnglatP1" placeholder="lng,lat"></el-input>
            <el-button type="primary" size="mini" @click="getMapCoord('p1')" style="margin-left:10px;">取点</el-button>
          </div>
          <div class="item">
            <label class="config_label">顶点1</label>
            <el-input size="mini" style="width:180px;" v-model="lnglatP2" placeholder="lng,lat"></el-input>
            <el-button type="primary" size="mini" @click="getMapCoord('p2')" style="margin-left:10px;">取点</el-button>
          </div>
        </div>
        <div class="box" v-show="interceptType=='center'">
          <div class="item">
            <label class="config_label">顶点1</label>
            <el-input size="mini" style="width:180px;" v-model="lnglatC1" placeholder="lng,lat"></el-input>
            <el-button type="primary" size="mini" @click="getMapCoord('c1')" style="margin-left:10px;">取点</el-button>
          </div>
          <div class="item">
            <label class="config_label">级别</label>
            <el-input size="mini" style="width:180px;" v-model="mapZoom" placeholder="地图缩放级别1~22"></el-input>
          </div>
        </div>
        <div class="item">
          <label class="config_label">宽度</label>
          <el-input size="mini" style="width:180px;" v-model="imgWidth" placeholder="1~1280"></el-input>
        </div>
        <div class="item">
          <label class="config_label">高度</label>
          <el-input size="mini" style="width:180px;" v-model="imgHeight" placeholder="1~1280"></el-input>
        </div>
        <el-button size="small" type="warning" @click="interceptMap()" style="margin-top:10px;">截取地图</el-button>
        <div class="tips">Tip:导出的底图为当前选择的底图</div>
      </div>
      <!-- 图层设置 -->
      <div class="tool_layerset" v-if="layerSetting">
        <el-checkbox-group v-model="displayLayers" @change="layerChange">
          <el-checkbox v-for="item in settingLayers" :label="item.value" :key="item.value">{{item.label}}</el-checkbox>
        </el-checkbox-group>
        <div class="tips">Tip:勾选显示对应图层</div>
      </div>
      <!-- 底图 -->
      <div class="tool_basicmap" v-if="showMapSelect">
        <el-radio-group v-model="curBasicmap" @change="changeBsicMap">
          <el-radio v-for="(item,i) in basicMap" :label="item.id" :key="i">{{item.name}}</el-radio>
        </el-radio-group>
      </div>
      <!-- 围栏 -->
...
import { chinaBoundary } from "@/assets/js/chinaBoundary.js"
export default {
  name: "city_map",
  props: {
    mapType:{ //使用地图的页面名称
  ...
  data() {
    return {
      chinaBoundaryData:{},
      map_ID:'mapbox_map',
      interceptState:false,//地图截取
      showMapSelect:false,//底图
      layerSetting:false,//图层设置
      ...
      curBasicmap:'cknihe9io0j4017odnittuw8e', //当前底图
      basicMap:[ //底图
        { id:'ckjxqgg190k8q17qib182syt7',name:'默认地图',url:'mapbox://styles/noconriander/ckjxqgg190k8q17qib182syt7' },
        { id:'ck3e3v9w74je21dod3xdvro6u',name:'卫星地图',url:'mapbox://styles/noconriander/ck3e3v9w74je21dod3xdvro6u' },
        { id:'ckhn5n4xt0a8j1apx3pvnqgei',name:'街道地图',url:'mapbox://styles/noconriander/ckhn5n4xt0a8j1apx3pvnqgei' },
        { id:'cknihe9io0j4017odnittuw8e',name:'暗色地图',url:'mapbox://styles/noconriander/cknihe9io0j4017odnittuw8e' },
        { id:'cknihl8w80j9817p5g64nycg7',name:'亮色地图',url:'mapbox://styles/noconriander/cknihl8w80j9817p5g64nycg7' },
        { id:'tiandi',name:'天地图',url:{
          "version": 8,
          "sources": {
              "raster-tiles": {
                  "type": "raster",
                  "tiles": ['http://t0.tianditu.gov.cn/DataServer?T=img_w&x={x}&y={y}&l={z}&tk=7f7be7d9fa88aa1e79cb744f76fa5a34'],
                  "tileSize": 256,
              }
          },
          "layers": [{
      ...
      measureTip:null,
      screenWidth:'',//屏幕宽度
      displayLayers:['regionLayer','landLabel'],//显示的图层
      settingLayers:[ //图层列表
        //{ label:'土地图层',value:'landLayer',layers:'' },
        { label:'城区图层',value:'regionLayer',layers:'plate_fill,plate_line,plate_labels' },
        { label:'土地标签',value:'landLabel',layers:'landpoint_label' },
        //{ label:'板块图层',value:'plateLayer',layers:'' },
      ],
      interceptType:'range',//地图截取方式
      lnglatP1:'',//范围顶点1
      lnglatP2:'',//范围顶点2
      lnglatC1:'',//中心点1
      imgWidth:'',//图像宽度
  ...
  beforeMount(){
    ...
    this.chinaBoundaryData = chinaBoundary
    console.log('地图设置')
    setTimeout(()=>{
      this.mapSet()
    },100)
  ...
  methods: {
    ...
    getMapCoord(pname){
      ...
      this.tipHTML.innerHTML = pname=='p1'?'在地图上选择顶点1':pname=='p2'?'在地图上选择顶点2':'在地图上选择中心点'
    ...
    interceptMap(){
      ...
      let access_token = 'pk.eyJ1Ijoibm9jb25yaWFuZGVyIiwiYSI6ImNrbXliMnRsejAyYWYyd28wc3RxYzN3ZjEifQ.9iGOZBik4yAVuNKMgrPPsA'
      let url = ''
      if(that.interceptType == 'range'){
        url = `https://api.mapbox.com/styles/v1/noconriander/${that.curBasicmap}/static/[${that.lnglatP1},${that.lnglatP2}]/${that.imgWidth}x${that.imgHeight}@2x?access_token=${access_token}`
      }else{
        url = `https://api.mapbox.com/styles/v1/noconriander/${that.curBasicmap}/static/${that.lnglatC1},${that.mapZoom?that.mapZoom:zoom}/${that.imgWidth}x${that.imgHeight}@2x?access_token=${access_token}`
      }
      that.$.ajax({
        type: "get",
        async: false, //同步请求
    ...
    layerChange(val){
      console.log(val)
      var map = this.local_map
      this.settingLayers.forEach(item=>{
        let flag = this.displayLayers.includes(item.value)
        let layers = item.layers.split(',')
        layers.forEach(a=>{
          let layerIsload = map.getLayer(a)
          if(layerIsload!==undefined){
            map.setLayoutProperty(a,'visibility',flag?'visible':'none')
          }
        })
      })
    },
    ...
    toolClick(index,item){ 
      if(index===7){ //用户使用手册
        window.open('http://oss.tzymap.com/投资云地图操作指南.pdf','_blank')
        return 
      }
    ...
    //地图切换
    changeBsicMap(id){
      let style = this.basicMap.filter(item=>item.id===id)[0].url
      this.base_map.setStyle(style)
    },
    ...
    mapSet() {
      ...
      if (!that.mbgl.supported()) {
        ...
        let style = that.basicMap.filter(item=>item.id===that.curBasicmap)[0].url
        ...
        //that.base_map.addControl(new that.mbgl.FullscreenControl());
        map.addControl(new that.mbgl.NavigationControl(), 'top-right');
        map.addControl(draw,'bottom-left')
        //地图是否加载完
        map.on('load', e => {
          if(that.mapType==='ledgerMap'){ //投资云地图
            that.$store.dispatch("setLedgerMap", map);
            that.$store.dispatch("setLedgerDraw", draw);
          }else if(that.mapType==='cityMap'){ //城市地图
            that.$store.dispatch("setCityMap", map);
          }else if(that.mapType==='macroMap'){ //宏观地图
            that.$store.dispatch("setMacroMap", map);
          }
          console.log('地图已加载')
          //加载中国边界线
          that.loadChinaBoundary()
          //geoserver图层加载
          ...
          //判断地图是否为登录加载
          if(!that.isLogin){
            if(that.mapType==='ledgerMap'){
              that.$store.dispatch('setMapisload',true)
            }else if(that.mapType==='cityMap'){
              that.$store.dispatch('setCityMapisload',true)
            }else if(that.mapType==='macroMap'){
              that.$store.dispatch('setMacroMapisload',true)
            }
          }else{
            setTimeout(_=>{
              that.$store.dispatch('setMapisload',true)
              that.$store.dispatch('setIsLogin',false)
            },1500)
          }
        })
        //图层事件-板块
    ...
    loadChinaBoundary(){
      var that = this
      let map = this.local_map
      map.addSource("china_boundary",{
        type:"geojson",
        data:that.chinaBoundaryData
      })
      map.addLayer({
        id: "china_boundary_line",
        type: "line",
    ...
    mapEvent(){
      ...
      //图层事件
      map.on('click',e=>{
        console.log('该点坐标:',e.lngLat.lng+','+e.lngLat.lat)
        if(that.isChoose){
          if(that.pName=='p1'){
            that.lnglatP1 = (e.lngLat.lng).toFixed(6)+','+(e.lngLat.lat).toFixed(6)
          }else if(that.pName=='p2'){
            that.lnglatP2 = (e.lngLat.lng).toFixed(6)+','+(e.lngLat.lat).toFixed(6)
          }else{
            that.lnglatC1 = (e.lngLat.lng).toFixed(6)+','+(e.lngLat.lat).toFixed(6)
          }
          that.isChoose = false
...
.map_box {
  ...
  .mapboxgl-popup-content{
    width: fit-content;
    max-width: 360px;
    background: linear-gradient(45deg, rgb(2,11,21,0.88), rgb(0,0,0,0.82));
...
.basemap_container{
  ...
  .tool_box{
    ...
    .tool_basicmap,.tool_enclosure,.tool_layerset,.tool_intercept{
      width: auto;
      height: 100%;
    ...
    .tool_basicmap,.tool_enclosure,.tool_layerset{
