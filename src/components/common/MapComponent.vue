<template>
  <div class="map_box">
    ...
    <div id="base_map" class="basemap_container"
    :style="{'height':mapType==='dataQuery'?(classBoxFlag?heigth_true:heigth_false):height_ohter}"></div>
    <div :id="map_ID"  class="map_container"
    :style="{'height':mapType==='dataQuery'?(classBoxFlag?heigth_true:heigth_false):height_ohter}"></div>
    <!-- 数据查询-土地图例 -->
    <div class="land_legend" v-if="queryMenuIndex===3&&mapType==='dataQuery'">
      <div class="deal_state" v-if="isListedLand">
        <div class="items" v-for="(item,index) in dealState" :key="index">
          <div class="point" :style="{'background':item.color}"></div>
          <div class="label">{{item.label}}</div>
        </div>
      </div>
      <div class="land_use">
        <div class="items" v-for="(item,index) in landUse" :key="index">
          <div class="color" v-if="!isListedLand" :style="{'background':item.color}"></div>
    ...
    <!-- 数据查询-管网图例及图层控制 -->
    <div class="pipe_legend_layer" v-if="pipeLegendFlag">
      <div class="layer">
        <div class="title">管网类型</div>
        <el-checkbox-group v-model="pipeList" @change="pipeChange">
          <el-checkbox v-for="(item,i) in pipeArr" :key="i" :label="item.value">{{item.label}}</el-checkbox>
        </el-checkbox-group>
      </div>
      <div class="legend">
        <div class="title">管网图例</div>
        <div class="cont">
          <div class="pipe_legend_item" v-for="(item,i) in pipeArr" :key="i">
    ...
    <div class="tool_box" v-if="cur_tool!=null">
      ...
      <div class="tool_color" v-if="cur_tool===1">
        <div class="color_evaluate" v-if="(mapType==='dataQuery'&&(queryMenuIndex===1||queryMenuIndex===0))||mapType==='plateGrade'">
          <div class="color_item" v-for="(item,index) in colors" :key="index">
            <el-color-picker v-model="item.value" size="mini"></el-color-picker>
            <div class="color_label">{{item.name}}</div>
          </div>
        </div>
        <div class="color_map">
...
// const DrawPolygon = require('mapbox-gl-draw-freehand-mode')
...
export default {
  ...
  data() {
    return {
      ...
      fill_opacity:0.6,//图层填充透明度
      border_opacity:0.6,//图层描边透明度

      curDistrictID:null,//当前小学poi点ID
      dealState:[ //土地交易状态
        {label:'已成交',color:'#fbb03b'},
        {label:'未成交',color:'#f56c6c'},
        {label:'交易中',color:'#00f3ff'}
      ],
      landUse:[ //土地用地性质
        {label:'纯住宅',sign:'R',color:'#ffff00',value:"house"},
        {label:'商住',sign:'CR',color:'#dd3601',value:"commerce_house"},
        {label:'商业',sign:'C',color:'#ff0000',value:"commerce"},
        {label:'工业',sign:'I',color:'#4d0000',value:"industry"},
        {label:'其他',sign:'E',color:'#3a8ee6',value:"other"}
      ],
      pipeLegendFlag:false,//管网图例显示
      pipeArr:[ //管网列表
        {label:"通信",value:"communication",color:"#fbb03b"},
        {label:"给水",value:"feedwater",color:"#f56c6c"},
        {label:"燃气",value:"gas",color:"#00f3ff"},
        {label:"供热",value:"heating",color:"#cd8cff"},
        {label:"电力",value:"power",color:"#33c377"},
        {label:"雨水",value:"rainwater",color:"#fbb03b"},
        {label:"污水",value:"sewage",color:"#00f3ff"},
        {label:"再生水",value:"reclaimed_water",color:"#f56c6c"},
        {label:"其他",value:"other",color:"#ccc"},
      ],
      pipeList:["communication","feedwater","gas","heating","power","rainwater","reclaimed_water","sewage","other"],//选择的管网类型
    };
  },
  beforeMount(){
  ...
  mounted() {
    //围栏功能控制，数据查询配套时显示
    if(this.queryMenuIndex===1&&this.mapType==="dataQuery"){
      this.tools[5].show = true
    }else{
      this.tools[5].show = false
    }
  },
  watch:{
    cur_tool:{
    ...
    classificationObj:{
      handler(newVal){
        if(this.queryMenuIndex===1&&this.mapType=="dataQuery"&&newVal.length>0){ //显示配套管网图例
          if(newVal[0].url==="pipe"){
            this.pipeLegendFlag = true  
          }else{
            this.pipeLegendFlag = false  
          }
        }else{
          this.pipeLegendFlag = false  
        }
    ...
    queryMenuIndex:{
      handler(newVal){
        //围栏功能控制，数据查询配套时显示
        if(newVal===1&&this.mapType==="dataQuery"){
          this.tools[5].show = true
        }else{
          this.tools[5].show = false
        }
  ...
  methods: {
    ...
    mapSet() {
      ...
      if (!that.mbgl.supported()) {
        ...
        that.$store.dispatch("setBaseLayerMap", baseMap);
        if(that.mapType==='dataQuery'){ //数据查询地图
          that.$store.dispatch("setMap", map);
        }else if(that.mapType==='ledgerMap'){ //台账地图
          that.$store.dispatch("setLedgerMap", map);
        }else if(that.mapType==='plateGrade'){ //板块评分地图
          that.$store.dispatch("setPlategradeMap", map);
        }else if(that.mapType==='pointGrade'){ //点地评分地图
          that.$store.dispatch("setPointgradeMap", map);
        }else if(that.mapType==='condition'){ //条件分析地图
          that.$store.dispatch("setConditionMap", map);
        }else if(that.mapType==='population'){ //人口数据地图
          that.$store.dispatch("setPopulationMap", map);
        }
    ...
    geoserverLayerLoad(){ //geoserver图层
      ...
      if(this.mapType==='dataQuery'){ //
        map = this.mapbox_map;
      }else if(this.mapType==='ledgerMap'){ //台账地图
        map = this.ledger_map;
      }else if(this.mapType==='plateGrade'){ //板块评分
        map = this.plategrade_map;
      }else if(this.mapType==='pointGrade'){ //点地评分
        map = this.pointgrade_map;
      }else if(this.mapType==='condition'){ //条件分析
        map = this.condition_map;
      }else if(this.mapType==='population'){ //人口数据
        map = this.population_map;
      }
      //板块中心点数据
      ...
      map.on("load", () => {
        ...
        map.addLayer({ //通用板块-填充
          ...
          filter: ["==", "type", this.mapType=="dataQuery"?this.plateType:'part'],
          paint: {
        ...
        //     "url": "http://47.104.165.136:8099/test/chengdukonggui.png",//图像地址
        //     "coordinates": [//图像显示4个点的位置信息
        //       [103.98504025603084, 30.56344266912296],
        //       [104.20514230053685, 30.563442669122807],
        //       [104.205142300537, 30.260942875904618],
        //       [103.98504025603081, 30.26094287590444]
        //     ]
        //   },
        // });
        //地图搜索点
    ...
    mapEvent(){
      ...
      map.on('mousemove','plate_fill',e=>{
        if (e.features.length > 0 && that.mapType==="dataQuery") {
      ...
      map.on('click','plate_fill',e=>{
        ...
        let centerStr = e.features[0].properties.center
        let plate_center = centerStr.split(' ')
        //点击飞往板块中心
        if(that.mapType!=='plateGrade'&&that.mapType!=='ledgerMap'&&that.cur_tool!==5&&that.mapType!=='pointGrade'){
          if(that.plateType==='region'){
            that.mapFlyTo(plate_center,9)
          }else{
            that.mapFlyTo(plate_center,12)
          }
        }
        //板块名片选择板块
        if(that.cardFlag&&that.queryMenuIndex!=1&&that.cur_tool!==5){ 
          
          let center = [e.lngLat.lng,e.lngLat.lat]
          //store存储选择的板块名片ID
          that.$store.dispatch("setPlateID", id);
          that.mapFlyTo(center,9)
        }
      })
      map.on('mouseup',(e)=>{
        //地理围栏
    ...
    pipeChange(){ //配套管网图层筛选
      ...
      let filter = ["in","type"].concat(this.pipeList)
      map.setFilter('poi_szgw', filter);
      map.setFilter('szgw_symbol', filter);
      map.setFilter('line_szgw_l', filter);
      map.setFilter('line_szgw_symbol', filter);
    },
  }
};
</script>
