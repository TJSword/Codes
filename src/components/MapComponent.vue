  ...
  <div class="map_box">
    ...
    <div class="map_tool">
      <div class="tool_item" @click="showMapSelect = !showMapSelect">
        <el-tooltip class="item" effect="light" content="底图选择" placement="bottom">
          <i class="iconfont icon-morenditu" :style="{'color':showMapSelect?'#f56c6c':'#333'}"></i>
        </el-tooltip>
      </div>
      <div class="tool_item" @click="ranging()">
        <el-tooltip class="item" effect="light" content="测距" placement="bottom">
          <i class="iconfont icon-ceju" :style="{'color':measureLengthState?'#f56c6c':'#333'}"></i>
        </el-tooltip>
      </div>
      <div class="tool_item" @click="chooseSite()">
        <el-tooltip class="item" effect="light" content="坐标拾取" placement="bottom">
          <i :class="['iconfont','icon-yingyong']" :style="{'font-weight':'bold','color':isChoosed?'#f56c6c':'#333'}"></i>
        </el-tooltip>
      </div>
    ...
    <!-- 地图容器 -->
    <div class="map_wrap" :style="{'z-index':mapMode?'9999':'0'}">
    ...
    <input type="text" ref="input" style="opacity:0;position:fixed;z-index:-99999">
  </div>
</template>
<script>
...
//组件
import Isocircle from './IsoCircle'
export default {
  name: "city_map",
  props: {
    mapType:{ //使用地图的页面名称
  ...
  methods: {
    chooseSite(){
      ...
      if(this.isChoosed){
        this.local_map.on('click',this.mapHadle)
      }else{
        this.local_map.off('click',this.mapHadle)
      }
    },
    
    mapHadle(e){
      this.$refs.input.value = e.lngLat.lng + ',' + e.lngLat.lat
    ...
    mapModeChange(){
      ...
      this.local_map.setLayoutProperty("plate_fill", "visibility", this.mapMode?'none':'visible');
      this.local_map.setLayoutProperty("plate_line", "visibility", this.mapMode?'none':'visible');
      this.local_map.setLayoutProperty("plate_labels", "visibility", this.mapMode?'none':'visible');
      this.local_map.setLayoutProperty("landOutline_fill", "visibility", this.mapMode?'none':'visible');
      this.local_map.setLayoutProperty("landOutline_line", "visibility", this.mapMode?'none':'visible');
      this.local_map.setLayoutProperty("landOutline_line_ps", "visibility", this.mapMode?'none':'visible');
      this.local_map.setLayoutProperty("landpoint_label", "visibility", this.mapMode?'none':'visible');
      this.local_map.setLayoutProperty("layer_point", "visibility", this.mapMode?'none':'visible');
      this.local_map.setLayoutProperty("layer_line", "visibility", this.mapMode?'none':'visible');
      this.local_map.setLayoutProperty("layer_line_symbol", "visibility", this.mapMode?'none':'visible');
      this.local_map.setLayoutProperty("cityNum", "visibility", this.mapMode?'none':'visible');
    },
    //测距
    ranging(){
    ...
    mapSet() {
      ...
      if (!that.mbgl.supported()) {
        ...
        var baseMap = new that.mbgl.Map({
          ...
          center: ["108.26150788281248", "31.69572366743195"],
          zoom: 4.8,
          pitch:0,
