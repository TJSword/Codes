<template>
  <div class="map_box" :style="{'z-index':mapMode?'999999':'0'}">
    ...
    <div id="houseData_map" class="basemap_container"></div>
...
export default {
  ...
  watch:{
    ...
    screenWidth: {
      handler(newValue) {
        if(this.houseData_map!==null){
          this.houseData_map.resize()
  ...
  methods: {
    ...
    mapSet() {
      ...
      if (!that.mbgl.supported()) {
        ...
        //that.houseData_map.addControl(new that.mbgl.FullscreenControl());
        map.addControl(new that.mbgl.NavigationControl(), 'top-right');
        // map.addControl(draw,'bottom-left')
        //地图是否加载完
        map.on('load', e => {
          that.$store.dispatch("setHouseDataMap", map);
          //geoserver图层加载
