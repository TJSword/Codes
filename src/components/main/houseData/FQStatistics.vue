      ...
      <div class="from_box">
        <el-form :inline="true" :model="formInline" size="small" style="text-align: start;">
          ...
          <!-- <el-form-item label="">
           ...
           <el-form-item label="">
            ...
            <el-form-item style="width:100%">
                <el-button style="width:220px"  type="warning"  @click="dialogVisible = true">房企组管理</el-button>
            </el-form-item>
            <el-form-item style="width:100%">
         ...
         <div :class="['list_mask',showMap?'list_mask_hide':'']">
            <div :class="['list_wrap',showRegionDetail?'list_wrap_hide':'']">
                <div class="instructions"> * 点击房企可查看区域详情</div>
                <el-table 
              :data="tableData"
             ...
             <el-table
                ...
                <el-table-column
                  ...
                  <template slot-scope="scope">
                    ...
                    <el-button type="text" size="small" @click="deleteFQGroup(scope.row)">删除</el-button>
                  </template>
                </el-table-column>
              </el-table>
              <div style="width:100%;text-align:right">
                <el-button type="text" @click="addFQ"><i class="el-icon-plus"></i>新增房企组</el-button>
              </div>
          </div>
        ...
        <el-dialog
                ...
                <el-table-column
                  ...
                  <template slot="header" slot-scope="scope">
                      <el-input
                        ...
                        :balabala="scope"
                        placeholder="输入关键字搜索">
                        <el-button slot="append" icon="el-icon-search" @click="searchEnterprise"></el-button>
                      </el-input>
                    </template>
                </el-table-column>
                <el-table-column
          ...
          <span slot="footer" class="dialog-footer">
            ...
            <el-button type="primary" @click="saveFQ">确 定</el-button>
          </span>
        </el-dialog>
        <div class="select_box">
          <el-select v-model="enterprise" size="small" style="width:180px" :clearable="false" @change="enterpriseChange">
            <el-option :label="item.name" :value="item.id" v-for="item in tableData" :key="item.value"></el-option>
          </el-select>
          <el-select v-model="indicator" size="small" style="width:180px;margin-left:20px;" :clearable="false" @change="indicatorChange">
...
export default {
  components:{
    ...
    watch:{
        ...
        avgSizes(newVal){this.formInline.avgSizes = newVal(',')},
        areaSizes(newVal){this.formInline.areaSizes = newVal(',')},
    ...
    methods:{
      ...
      exportExcel(){
        let url = `${process.env.BASE_URL}/dms/sc/building/newhouse/export?modelType=enterprise_count&expPage=${this.showRegionDetail?'second':'first'}&token=${localStorage.getItem('token')}`
        window.open(url)
      },
      hotList(){
    ...
                ],
      ...
      handleAdd(row,index){
        ...
        this.$set(this.enterpriseList[index],'isChoose','1')
        this.FQChooseList.push(row)
      },
      searchEnterprise(){
        console.log(this.search)
        House.searchEnterprise(this.search).then(res=>{
      ...
      handleChoose(row){
        ...
        let params = {id:row.id,isChoose:'1',type:'1'}
      ...
      getViewList(){
          ...
          this.showLoading = true
          House.getFQInfo(this.formInline).then(res=>{
            this.showLoading = false
            this.showRegionDetail = false
