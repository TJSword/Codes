<template>
  <div :class="['regionDetail_wrap',ani?'regionDetail_wrap_active':'']">
    ...
    <dv-border-box-12 :class="['list_mask',ani?'list_mask_active':'']">
        ...
        <el-form-item label="" v-if="type==='region'">
            <el-select v-model="formInlineRegionno" placeholder="区域" clearable multiple :collapse-tags="true">
            <el-option :label="item.name" :value="item.id" v-for="item in regionnoArr" :key="item.id"></el-option>
            </el-select>
        </el-form-item>
         <el-form-item label="" v-if="type==='part'">
            <el-select v-model="formInlinePartno" placeholder="区域" clearable multiple :collapse-tags="true">
            <el-option :label="item.name" :value="item.id" v-for="item in regionnoArr" :key="item.id"></el-option>
            </el-select>
        </el-form-item>
        <el-form-item label="">
        ...
        <el-form-item>
            <el-date-picker type="month" value-format="yyyy-MM-dd" placeholder="开始日期" v-model="formInline.startDate"  style="width: 100%;"></el-date-picker>
        </el-form-item>
            <el-form-item>
            <el-date-picker type="month" value-format="yyyy-MM-dd" placeholder="结束日期" v-model="formInline.endDate" style="width: 100%;"></el-date-picker>
        </el-form-item>
            <el-form-item>
...
export default {
  ...
  watch:{
    ...
    //   },
      ...
      formInlinePartno(newVal){
          this.formInline.partnos = newVal.join(',')
      }
  },
  methods:{
  ...
  created(){
        ...
        if(this.type === 'region'){
          House.getRegionnos().then(res=>this.regionnoArr = res.data.data.searchRst)
          this.formInlineRegionno = [this.regionno];
          this.formInline.regionnos = this.regionno;
        }else if(this.type === 'part'){
          console.log(this.regionno)
          House.getPartNos().then(res=>this.regionnoArr = res.data.data)
          this.formInlinePartno = [this.regionno];
          this.formInline.partnos = this.regionno;
        }
