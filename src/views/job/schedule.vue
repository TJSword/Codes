  ...
  methods: {
    ...
    pauseHandle(id?: string) {
      ...
      this.$confirm(this.$t("prompt.info", { handle: this.$t("schedule.pause") }), this.$t("prompt.title"), {
        ...
        .then(() => {
          baseService.put("/job/schedule/pause", id ? [id] : this.dataListSelections && this.dataListSelections.map((item: IObject) => item.id)).then((res) => {
            if (res.code !== 0) {
    ...
    resumeHandle(id?: string) {
      ...
      this.$confirm(this.$t("prompt.info", { handle: this.$t("schedule.resume") }), this.$t("prompt.title"), {
        ...
        .then(() => {
          baseService.put("/job/schedule/resume", id ? [id] : this.dataListSelections && this.dataListSelections.map((item: IObject) => item.id)).then((res) => {
            if (res.code !== 0) {
              return this.$message.error(res.msg);
            }
            this.$message({
    ...
    runHandle(id?: string) {
      ...
      this.$confirm(this.$t("prompt.info", { handle: this.$t("schedule.run") }), this.$t("prompt.title"), {
        ...
        .then(() => {
          baseService.put("/job/schedule/run", id ? [id] : this.dataListSelections && this.dataListSelections.map((item: IObject) => item.id)).then((res) => {
            if (res.code !== 0) {
              return this.$message.error(res.msg);
            }
            this.$message({
