...
export default defineComponent({
  ...
  computed: {
    dataRule() {
      return {
        smsCode: [{ required: true, message: this.$t("validate.required"), trigger: "blur" }],
        "config.aliyunAccessKeyId": [{ required: true, message: this.$t("validate.required"), trigger: "blur" }],
        "config.aliyunAccessKeySecret": [{ required: true, message: this.$t("validate.required"), trigger: "blur" }],
        "config.aliyunSignName": [{ required: true, message: this.$t("validate.required"), trigger: "blur" }],
        "config.aliyunTemplateCode": [{ required: true, message: this.$t("validate.required"), trigger: "blur" }],
        "config.qcloudAppId": [{ required: true, message: this.$t("validate.required"), trigger: "blur" }],
        "config.qcloudAppKey": [{ required: true, message: this.$t("validate.required"), trigger: "blur" }],
        "config.qcloudSignName": [{ required: true, message: this.$t("validate.required"), trigger: "blur" }],
        "config.qcloudTemplateId": [{ required: true, message: this.$t("validate.required"), trigger: "blur" }],
        "config.qiniuAccessKey": [{ required: true, message: this.$t("validate.required"), trigger: "blur" }],
        "config.qiniuSecretKey": [{ required: true, message: this.$t("validate.required"), trigger: "blur" }],
        "config.qiniuTemplateId": [{ required: true, message: this.$t("validate.required"), trigger: "blur" }]
      };
    }
  },
  watch: {
  ...
  methods: {
    ...
    // 获取信息
    getInfo() {
      baseService.get(`/message/sms/${this.dataForm.id}`).then((res) => {
        if (res.code !== 0) {
    ...
    dataFormSubmitHandle() {
      this.$refs["dataFormRef"].validate((valid: boolean) => {
        ...
        (!this.dataForm.id ? baseService.post : baseService.put)("/message/sms", this.dataForm).then((res) => {
          if (res.code !== 0) {
            return this.$message.error(res.msg);
          }
          this.$message({
