<template>
  <div class="map_pipe">
    ...
    <div :id="map_ID" 
    class="map_container"
    :style="{'height':mapType==='dataQuery'?(classBoxFlag?heigth_true:heigth_false):height_ohter}"
    ></div>
    <!-- 数据查询-管网图例及图层控制 -->
    <div class="pipe_legend_layer">
      <div class="layer">
...
export default {
  ...
  computed: {
    ...mapGetters(["cityCode","mapbox_map","ledger_map","plategrade_map","condition_map","plateCenter","classBoxFlag","plateData","cardFlag","plate_color","plateType","queryMenuIndex","isListedLand","classificationObj"])
  },
  methods: {
    mapSearch(val){
    ...
    mapSet() {
      ...
      if (!that.mbgl.supported()) {
        ...
        }else if(that.mapType==='customGrade'){ //自定义评分地图
          that.$store.dispatch("setCustomgradeMap", map);
    ...
    geoserverLayerLoad(){ //geoserver图层
      ...
      //配套学区MultiPolygon
      var polygon_district = url_head+workSpace_layer+'school_district'+url_end
      map.on("load", () => {
    ...
    mapEvent(){
      ...
      map.on('mousemove','plate_fill',e=>{
        if (e.features.length > 0 && this.mapType==="dataQuery") {
      ...
      map.on('click','plate_fill',e=>{
        ...
        //点击飞往板块中心
        if(this.mapType!=='plateGrade'&&this.cur_tool!==5){
          if(this.plateType==='region'){
            this.mapFlyTo(plate_center,9)
          }else{
            this.mapFlyTo(plate_center,12)
          }
        }
        //板块名片选择板块
        if(this.cardFlag&&this.queryMenuIndex!=1&&this.cur_tool!==5){ 
          
          let center = [e.lngLat.lng,e.lngLat.lat]
          //store存储选择的板块名片ID
          this.$store.dispatch("setPlateID", id);
          this.mapFlyTo(center,9)
        }
      })
      map.on('mouseup',(e)=>{
        //地理围栏
        if(this.cur_tool===5&&this.map_draw.getAll().features.length>0){
          let feaCol = this.map_draw.getAll()
          console.log(feaCol)
          //移除dom
          let measure_result = document.getElementsByClassName("measure_result");
          let l = measure_result.length
          ...
          //存储围栏坐标
          let coorStr = ''
          if(this.curEnclosure==='circle'){ //半径圆
            ...
            if(radiusInKm){
              ...
              this.addMeasureRes(coords,`半径:${radius} km`)
              this.local_map.getCanvas().style.cursor = '';
            }
            //存储围栏坐标
            ...
            console.log(coorStr)
          }else if(this.curEnclosure==='polygon'){  //自定义多边形
            //添加边长marker
            let feaCol = this.map_draw.getAll()
            ...
            if(points.length>2){
              ...
              points.forEach((item,i)=>{
                ...
                if(i<points.length-1){
                  ...
                  if(len>0){
                    this.addMeasureRes(midPointCoor,`${len} km`)
              ...
              if(points.length>3){
                ...
                this.addMeasureRes(featureCenter,`${area2} k㎡`)
                //存储围栏坐标
                let coordinates = feaCol.features[0].geometry.coordinates[0]
                coordinates.forEach(item=>{
                  coorStr += `${item[0]},${item[1]};`
          ...
          }else if(this.curEnclosure==='isochronous'){ //等时圈
            console.log(e)
            let coor = feaCol.features[0].geometry.coordinates
            if(coor.length>0){
              this.isoPoint = coor[0].toFixed(6)+','+coor[1].toFixed(6)
              this.addMeasureRes(coor,`起始点`)
            }else{
              this.isoPoint = e.lngLat.lng.toFixed(6)+','+e.lngLat.lat.toFixed(6)
              this.addMeasureRes([e.lngLat.lng,e.lngLat.lat],`起始点`)
            }
            this.local_map.getCanvas().style.cursor = '';
          }
        }
      ...
      map.on('wheel',e=>{
        this.mapSize = map.getZoom().toFixed(2)
      })
      var searchPoiPopup = new this.mbgl.Popup({
        closeButton: false,
        closeOnClick: false,
      });
...
.map_pipe {
  ...
  .mapboxgl-popup-content{
    ...
    min-width: 200px;
    max-width: 300px;
    border-top: rgba(18, 150, 219, 0.6) 1px solid;
    border-bottom: rgba(18, 150, 219, 0.6) 1px solid;
    border-radius: 8px;
    color: #fff;
    .item_head{
      color: #3cff43;
