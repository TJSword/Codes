<template>
  ...
  <div class="cd_map">
    ...
    <!-- 物业类型 -->
    <div class="property_box" v-show="curProIndex!==null&&showCdSelect">
    ...
    <!-- 竞品配置 -->
    <div class="cd_set" v-show="showCdSelect">
      <!-- 权重调整 -->
      <div class="cd_box">
        <div class="cd_item">
          <div class="name" :style="{'background':curProIndex==-1?'rgba(34, 133, 236, 0.8)':'rgba(37, 80, 86, 0.8)'}">
            <div class="name_label" @click="showPropertyBox(-1,landPoint.name)">{{landPoint.name}}</div>
          </div>
          <!-- <div class="weight" style="font-size:12px">点击按钮选择物业类型及建筑形态</div> -->
        </div>
        <div class="cd_item" v-for="(item,i) in cd_list" :key="item.id">
          <div class="name" :style="{'background':curProIndex==i?'rgba(34, 133, 236, 0.8)':'rgba(37, 80, 86, 0.8)'}">
            <div class="name_label" @click="showPropertyBox(i,item.name)">{{item.name}}</div>
          </div>
          <!-- <div class="weight">
    ...
    <!-- 操作按钮 -->
    <div class="option_box" v-show="showCdSelect">
      <el-button size="mini" type="info" @click="clearCd()">清空竞品</el-button>
      <el-button size="mini" type="success" @click="subProperties()">确认竞品</el-button>
    </div>
  </div>
</template>
<script>
  ...
  watch:{
    showCdSelect:{
      handler(newVal){
        ...
        if(newVal){
          let map = this.CD_map
          let CD_source = map.getSource('CD_source')
          if(CD_source!==undefined){
            map.setLayoutProperty('project_point','visibility','visible')
            map.setLayoutProperty('cd_point','visibility','visible')
          }
          this.getCdProperty()
        }
      },
          ...
          if(res.data.code===0){
            ...
            .then(res=>{
              if(res.data.code===0){
                ...
                let CD_source = map.getSource('CD_source')
                if(CD_source!==undefined){
                  map.setLayoutProperty('project_point','visibility','none')
                  map.setLayoutProperty('cd_point','visibility','none')
                }
              }
            })
          }
