...
import { registerToRouter, toLangRoutes } from "@/utils/router";
    ...
    if (store.state.routes.length) {
      ...
      if (token) {
        ...
        store.dispatch({ type: "initApp" }).then((res: Array<RouteRecordRaw>) => {
          const mergeRoute = toLangRoutes(baseRoutes, initI18n.global.t)
            .concat(res)
            .concat(errorRoute);
          ...
          store.commit("updateState", { appIsReady: true, routes: mergeRoute });
          setTimeout(() => {
